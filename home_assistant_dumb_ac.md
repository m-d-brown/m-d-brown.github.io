...
doesn't provide a signal as to whether the A/C is on or off, preventing the use
of a standard thermometer controller.
...
With this, the timer starts the A/C unit (through the Broadlink) when the timer
itself beings. When the timer ends, it either refreshes itself (because more
cooling is needed) or turns off the A/C unit.
...
- An IR blaster with an integration like
  ...
  called `remote.broadlink_remote` here. The device must have been set up with
...
- [SmartIR](https://github.com/smartHomeHub/SmartIR) for wrapping the controls
  in a proper
...
- Using a door sensor on the A/C exhaust louvers to detect if they are open thus
  to sense if the A/C unit is running. (My unit is hardwired so I don't to
  detect activity using electric draw.)
...
continue running for another cycle. Otherwise, the timer is going away so turn
off the A/C.
